...
- **ArrayBuffer:** `[Uint8Contents]` and `byteLength` are displayed for `ArrayBuffer` objects. For example, `ArrayBuffer { [Uint8Contents]: <2a 00 00 00 00 00 00 00>, [byteLength]: 8 }`. If the `ArrayBuffer` is [detached](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/ArrayBuffer#transferring_arraybuffers), it is displayed as `ArrayBuffer { (detached), [byteLength]: 0 }`.
